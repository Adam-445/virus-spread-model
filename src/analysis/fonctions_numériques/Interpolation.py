...
class Interpolation:
    ...
    def difference_divisee_newton(self):
        ...
        for j in range(1, n):
            for i in range(n - 1, j - 1, -1):  # Utiliser les valeurs d'index pour x_i et x_(i-j)
                x_i = self.x[i]
                x_i_j = self.x[i - j]
                coefficients[i] = (coefficients[i] - coefficients[i - 1]) / (x_i - x_i_j) # Calcul de la différence divisée de Newton
    ...
    def natural_cubic_spline(self, x_val):
        ...
        c = [0] * (n + 1)  # Coefficients c des splines (suite de polynomes)initialisation 
        ...
        for j in range(n-1, -1, -1):  # Résolution  pour trouver c, b et d par algorithme de Thomas Matriciel
            c[j] = z[j] - mu[j] * c[j+1]
